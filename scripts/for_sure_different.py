...
def compare_and_move_file(file_path_a, file_path_b, output_a, output_b, diff_output):
    size_a = file_path_a.stat().st_size
    size_b = file_path_b.stat().st_size

    if size_a != size_b:
        relative_path = file_path_a.relative_to(file_path_a.parents[3])
        ...
        shutil.move(file_path_a, output_path_a)
        shutil.move(file_path_b, output_path_b)
